...
class hqafDataset(Dataset):
    def __init__(self,dpath,file_path,input_type,folds,qtest=False, diff_level=None):
        ...
        questions_avgusetimes_path = dpath+f'/questions_avgusetimes.csv'
        if not os.path.exists(skills_difficult_path) or not os.path.exists(questions_difficult_path):
            ...
            # df = df[df['fold'] != 0]  # 保留 fold 为 1, 2, 3, 4 的行
        ...
        if not os.path.exists(processed_data):
            ...
            if self.qtest:
                self.dori, self.dqtest = self.__load_data__(self.sequence_path,skills_difficult_path,questions_difficult_path, questions_avgusetimes_path, folds)
                save_data = [self.dori, self.dqtest]
            else:
                self.dori = self.__load_data__(self.sequence_path,skills_difficult_path,questions_difficult_path, questions_avgusetimes_path, folds)
                save_data = self.dori
    ...
    def __load_data__(self,sequence_path,sds_path,qds_path,qus_path,folds,pad_val=-1):
        dori = {"qseqs":[],"cseqs":[],"rseqs":[],"tseqs":[],"utseqs":[],"smasks": [],"sdseqs":[],"qdseqs":[],"quseqs":[],"utTseqs":[],"pTseqs":[]}
        ...
        with open(qus_path,'r',encoding="UTF8") as f:
            reader = csv.reader(f)
            qus_keys = next(reader)
            qus_vals = next(reader)
            for i in range(len(qus_keys)):
                qus[int(qus_keys[i])] = int(qus_vals[i])
        interaction_num = 0
        dqtest = {"qidxs":[],"rests":[], "orirow":[]}
        sds_keys = [int(_) for _ in sds_keys]
        qds_keys = [int(_) for _ in qds_keys]
        qus_keys = [int(_) for _ in qus_keys]
        for i,row in df.iterrows():
            ...
            if "questions" in self.input_type:
                ...
                for j in temp:
                    if j == -1:
                        temp_2.append(-1)
                    elif j not in qus_keys:
                        temp_2.append(1)
                    else:
                        temp_2.append(int(qus[j]))
                dori["quseqs"].append(temp_2)
            if "timestamps" in row:
                dori["tseqs"].append([int(_) for _ in row["timestamps"].split(",")])
            if "usetimes" in row:
                dori["utTseqs"].append([int(_) for _ in row["usetimes"].split(",")])
            if "usetimes" in row:
                # dori["utseqs"].append([int(_) for _ in row["usetimes"].split(",")])
                dori["utseqs"].append([int(float(_)) for _ in row["usetimes"].split(",")])
            if "type" in row:
                # dori["utseqs"].append([int(_) for _ in row["usetimes"].split(",")])
                dori["pTseqs"].append([int(_) for _ in row["type"].split(",")])
